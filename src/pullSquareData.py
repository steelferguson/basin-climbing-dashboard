...
def transform_payments_data(df):
    ...
    df[['revenue_category', 'membership_size', 'membership_freq', 'is_founder', 'is_free_membership']] = \
        df['Description'].apply(lambda x: pd.Series(categorize_transaction(x)))
...
def pull_square_payments_data_raw(square_token, location_id, end_time, begin_time, limit):
    ...
    for order in orders_list:
        ...
        total_money = order.get('total_money', {}).get('amount', 0)  # Total amount
        pre_tax_money = order.get('net_amounts', {}).get('amount', 0)  # Pre-tax amount
        tax_money = order.get('total_tax_money', {}).get('amount', 0)  # Tax amount
        discount_money = order.get('total_discount_money', {}).get('amount', 0)  # Discount amount
        currency = order.get('total_money', {}).get('currency')  # Currency
...
def pull_and_transform_square_payment_data():
    ...
    begin_time = one_year_ago.strftime('%Y-%m-%dT%H:%M:%SZ')
...
if __name__ == "__main__":
    pull_and_transform_square_payment_data()
